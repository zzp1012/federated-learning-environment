...
# ************************************************************************************************************ #
        if self.args.FPF == 1:
            self.FPF_cal = self.FPF1_cal
        else:
            self.FPF_cal = self.FPF2_cal
# ************************************************************************************************************ #
...
# ************************************************************************************************************ #
    def feedback(self, loss_locals, FPF_idx_lst):
        # setup client
        hostname = socket.gethostname()
        client_socket = socket.socket()
        client_socket.connect((hostname, 8999))

        # send time_counter & local_losses & FPF1_index
        message = str(self.time_counter) + ',' + str(loss_locals)[1:-1].replace(',', '') + ',' +  str(list(FPF_idx_lst.ravel()))[1:-1].replace(',', '')
...
# ************************************************************************************************************ #
        ...
        FPF_idx_lst = np.zeros((1, client_num_in_total)) # maintain a lst for FPF1 indexes
        ...
        for para in self.model_global.state_dict().keys():
            ...
            A_mat[para] = np.zeros(weight_shape) # initial the value of A with zero.
...
# ************************************************************************************************************ #
            ...
            self.feedback(loss_locals, FPF_idx_lst)
...
# ************************************************************************************************************ #
            ...
            FPF_idx_lst = self.FPF_cal(local_w_lst)
